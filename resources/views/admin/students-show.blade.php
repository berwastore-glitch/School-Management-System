...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <div class="d-flex gap-2">
        ...
        <a href="{{ route('admin.students.marks', $student) }}" class="btn btn-success btn-sm"><i class="fas fa-pen me-1"></i>Enter Marks</a>
        <a href="{{ route('admin.students.edit', $student) }}" class="btn btn-warning btn-sm"><i class="fas fa-edit me-1"></i>Edit</a>
        <a href="{{ route('admin.students') }}" class="btn btn-secondary btn-sm"><i class="fas fa-arrow-left me-1"></i>Back</a>
    </div>
</div>
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-white d-flex justify-content-between align-items-center">
                ...
                <div class="d-flex align-items-center gap-2">
                    ...
                    <a href="{{ route('admin.students.marks', $student) }}" class="btn btn-sm btn-outline-success"><i class="fas fa-pen me-1"></i>Enter Marks</a>
                </div>
            </div>
                        ...
                        <table class="table table-hover mb-0">
                            ...
                            <tbody>
                                ...
                                <tr data-term="{{ $result->term }}">
                                    ...
                                    <td>
                                        <form method="POST" action="{{ route('admin.students.marks.store', $student) }}" class="d-flex align-items-center gap-1">
                                            @csrf
...
function calculateSubject(subjData) {
    const m = subjData.mid;
    const e = subjData.end;
    const midMarks    = m ? safeNum(m.marks_obtained) : 0;
    const midTotal    = m ? safeNum(m.exam.total_marks) : 0;
    const endMarks    = e ? safeNum(e.marks_obtained) : 0;
    const endTotal    = e ? safeNum(e.exam.total_marks) : 0;
    const marksObtained = midMarks + endMarks;
    const totalMarks    = midTotal + endTotal;
    const percentage    = calcPercentage(marksObtained, totalMarks);
    const grade         = calcGrade(percentage);
    return { midMarks: midMarks, midTotal: midTotal, endMarks: endMarks, endTotal: endTotal, marksObtained: marksObtained, totalMarks: totalMarks, percentage: percentage, grade: grade, hasData: m !== null || e !== null };
}
...
function calculateReport(subjectMap) {
    ...
    Object.keys(subjectMap).sort().forEach(function(subjName) {
        ...
        if (calc.hasData) {
            totalMidObt += calc.midMarks; totalMidFull += calc.midTotal;
            totalEndObt += calc.endMarks; totalEndFull += calc.endTotal;
            grandObt += calc.marksObtained; grandFull += calc.totalMarks;
        }
    });
...
function calculateRanking(classmates, currentStudentId, term) {
    ...
    classmates.forEach(function(stu) {
        ...
        var pct = full > 0 ? (obt / full) * 100 : 0;
        rankings.push({ id: stu.id, name: stu.first_name + ' ' + stu.last_name, pct: pct, obt: obt, full: full });
    });
    rankings.sort(function(a, b) { return b.pct - a.pct; });
    var position = 0;
    for (var i = 0; i < rankings.length; i++) {
        if (rankings[i].id === currentStudentId) { position = i + 1; break; }
    }
    return { position: position, total: rankings.length, rankings: rankings };
}

// ─── QR Code Generator (minimal SVG) ────────────────────────────────────────
...
function generateQRSVG(text, size) {
    ...
    var cells = text.length > 40 ? 25 : 21;
    ...
    // Timing patterns
    for (var i = 8; i < cells - 8; i++) {
        if (i % 2 === 0) { ctx.fillRect(i * cellSize, 6 * cellSize, cellSize, cellSize); ctx.fillRect(6 * cellSize, i * cellSize, cellSize, cellSize); }
    }
    // Data area
    for (var r = 0; r < cells; r++) {
        for (var c = 0; c < cells; c++) {
            if ((r < 9 && c < 9) || (r < 9 && c >= cells - 8) || (r >= cells - 8 && c < 9)) continue;
            if (r === 6 || c === 6) continue;
...
function gradeColor(grade) {
    switch (grade) {
        case 'A+': return '#0F7B3F'; case 'A': return '#1A8C4E'; case 'B': return '#2E7D32';
        case 'C': return '#D4A24C'; case 'D': return '#E67E22'; case 'F': return '#C0392B'; default: return '#999';
    }
}

function gradeBg(grade) {
    switch (grade) {
        case 'A+': case 'A': return '#E8F5E9'; case 'B': return '#F1F8E9'; case 'C': return '#FFF8E1';
        case 'D': return '#FFF3E0'; case 'F': return '#FFEBEE'; default: return '#F5F5F5';
    }
}
...
function printReport() {
    ...
    var currentStudentId = {{ $student->id }};
    var ranking = calculateRanking(classmates, currentStudentId, term);
    ...
    var studentEmail = '{{ $student->user ? $student->user->email : "N/A" }}';
    var studentPhone = '{{ $student->phone ?? "N/A" }}';
    ...
    var qrData = 'Student:' + studentId + '|Term:' + term + '|Year:' + year + '|Grade:' + report.overallGrade + '|Pct:' + (report.overallPct || 0) + '%';
    ...
    report.subjects.forEach(function(s, i) {
        ...
        var midDisplay  = c.hasData && c.midTotal > 0 ? c.midMarks + ' / ' + c.midTotal : '\u2014';
        var endDisplay  = c.hasData && c.endTotal > 0 ? c.endMarks + ' / ' + c.endTotal : '\u2014';
        var pctDisplay  = c.percentage !== null ? c.percentage + '%' : '\u2014';
        var gradeDisplay = c.hasData ? c.grade : '\u2014';
        var gc = gradeColor(c.grade);
        var gb = gradeBg(c.grade);
        ...
        rows += '<tr style="background:' + bgColor + '">'
            ...
            + '<td style="padding:10px 12px;border-bottom:1px solid #E8ECF0;text-align:center;font-size:13px;color:#475569">' + midDisplay + '</td>'
            + '<td style="padding:10px 12px;border-bottom:1px solid #E8ECF0;text-align:center;font-size:13px;color:#475569">' + endDisplay + '</td>'
            ...
            + '<td style="padding:10px 12px;border-bottom:1px solid #E8ECF0;text-align:center;font-size:13px;color:#475569">' + pctDisplay + '</td>'
            + '<td style="padding:10px 12px;border-bottom:1px solid #E8ECF0;text-align:center"><span style="display:inline-block;padding:3px 10px;border-radius:20px;font-size:12px;font-weight:600;color:' + gc + ';background:' + gb + '">' + gradeDisplay + '</span></td>'
            + '<td style="padding:10px 12px;border-bottom:1px solid #E8ECF0;text-align:center;font-size:11px;color:#64748B;font-style:italic">' + remark + '</td>'
            + '</tr>';
    });

    var grandPctDisplay  = report.overallPct !== null ? report.overallPct + '%' : '\u2014';
    var grandGradeDisplay = report.grandFull > 0 ? report.overallGrade : '\u2014';
    var grandGc = gradeColor(report.overallGrade);
    var grandGb = gradeBg(report.overallGrade);
    var distLabel = report.overallPct >= 80 ? 'Distinction' : report.overallPct >= 60 ? 'Merit' : report.overallPct >= 50 ? 'Pass' : 'Below Average';

    var html = '<!DOCTYPE html><html><head>'
        + '<meta charset="UTF-8"><title>Report Card \u2014 ' + studentName + '</title>'
        + '<link rel="preconnect" href="https://fonts.googleapis.com">'
        + '<link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&family=Poppins:wght@400;500;600;700;800&display=swap" rel="stylesheet">'
        + '<style>'
        + '@page { size: A4 portrait; margin: 10mm 12mm; }'
        + '* { margin:0;padding:0;box-sizing:border-box; }'
        + 'body { font-family:"Inter",sans-serif; color:#1E293B; background:#FAFAFA; -webkit-print-color-adjust:exact; print-color-adjust:exact; }'
        ...
        + '.sum-card .big.gold { color:#D4A24C; }'
        + '.sum-card .sub { font-size:11px;color:#64748B;margin-top:3px; }'
        ...
        + '.sig-line { border-bottom:2px solid #0F2D52;margin:36px 0 6px 0; }'
        + '.sig-name { font-size:10px;font-weight:600;color:#0F2D52; }'
        + '.sig-role { font-size:9px;color:#94A3B8;text-transform:uppercase;letter-spacing:1px; }'
        ...
        + '.qr-box img { width:80px;height:80px;border-radius:8px;border:1px solid #E8ECF0; }'
        ...
        + '<div class="sum-card"><h4>Overall</h4><div class="grade-pill" style="color:' + grandGc + ';background:' + grandGb + '">' + grandGradeDisplay + '</div><div class="sub" style="margin-top:4px">' + grandPctDisplay + ' \u2022 ' + distLabel + '</div></div>'
        + '</div>'
        ...
        + '<div class="att-item"><div class="att-num att-pct">' + attendPct + '%</div><div class="att-lbl">Attendance</div></div>'
        + '</div>'

        + '<div class="sec-title">Remarks</div>'
        + '<div class="remarks-grid">'
        + '<div class="remark-box"><h4>Class Teacher\u2019s Remarks</h4><div class="remark-line"></div></div>'
        + '<div class="remark-box"><h4>Principal\u2019s Remarks</h4><div class="remark-line"></div></div>'
        + '</div>'
        ...
        + '<div class="stamp-box"><span>School<br>Stamp</span></div>'
        + '</div>'
        + '<div class="sig-box"><div class="sig-line"></div><div class="sig-name">Principal</div><div class="sig-role">Signature & Seal</div></div>'
        + '</div>'
        ...
        + '<div style="font-size:8px;color:#CBD5E1;margin-top:2px;">Verification ID: ' + studentId + '-' + term.replace(/\s/g,'') + '-' + year + '</div></div>'
        + '</div>'

        + '</div></body></html>';

    var printWindow = window.open('', '_blank', 'width=794,height=1123');
